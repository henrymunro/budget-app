...
Parser = function(options = {}) {
  ...
  this._ = {
    ...
    rowDelimiterLength: this.options.rowDelimiter
      ? Math.max(
          ...this.options.rowDelimiter.map(function(v) {
            return v.length;
          })
        )
      : void 0
...
Parser.prototype._flush = function(callback) {
  ...
  if (this._.quoting) {
    this.emit(
      "error",
      new Error(`Quoted field not terminated at line ${this.lines + 1}`)
    );
    return;
  }
...
Parser.prototype.__push = function(line) {
  ...
  if (line.length === 1 && line[0] === "") {
    ...
    if (this.options.relax_column_count) {
      ...
      return Error(
        `Number of columns on line ${this.lines} does not match header`
      );
    } else {
...
Parser.prototype.__write = function(chars, end) {
  ...
  while (i < l) {
    ...
    if (!end) {
      ...
      if (
        (!this.options.rowDelimiter && i + 3 > l) ||
        (!this._.commenting &&
          l - i < this.options.comment.length &&
          this.options.comment.substr(0, l - i) === remainingBuffer) ||
        (this.options.rowDelimiter &&
          l - i < this._.rowDelimiterLength &&
          this.options.rowDelimiter.some(function(rd) {
            return rd.substr(0, l - i) === remainingBuffer;
            // Skip if the remaining buffer can be row delimiter following the closing quote
          })) ||
        (this.options.rowDelimiter &&
          this._.quoting &&
          l - i < this.options.quote.length + this._.rowDelimiterLength &&
          this.options.rowDelimiter.some(rd => {
            return (
              (this.options.quote + rd).substr(0, l - i) === remainingBuffer
            );
            // Skip if the remaining buffer can be delimiter
            // Skip if the remaining buffer can be escape sequence
          })) ||
        (l - i <= this.options.delimiter.length &&
          this.options.delimiter.substr(0, l - i) === remainingBuffer) ||
        (l - i <= this.options.escape.length &&
          this.options.escape.substr(0, l - i) === remainingBuffer)
      ) {
        break;
      }
    }
    ...
    if (!this._.commenting && char === this.options.escape) {
      ...
      if (
        !(escapeIsQuote && this._.field == null && !this._.quoting) &&
        (isEscape || isQuote)
      ) {
        i++;
    ...
    if (!this._.commenting && char === this.options.quote) {
      if (this._.quoting) {
        ...
        areNextCharsDelimiter =
          chars.substr(i + 1, this.options.delimiter.length) ===
        ...
        if (
          ...
          if (this.options.relax) {
            ...
            return Error(
              `Invalid closing quote at line ${this.lines +
                1}; found ${JSON.stringify(
                this._.nextChar
              )} instead of delimiter ${JSON.stringify(this.options.delimiter)}`
            );
          }
        } else {
          this._.quoting = false;
    ...
    if (
      ...
      chars.substr(i, this.options.comment.length) === this.options.comment
    ) {
    ...
    isDelimiter =
      chars.substr(i, this.options.delimiter.length) === this.options.delimiter;
    if (
      !this._.commenting &&
      !this._.quoting &&
      (isDelimiter || isRowDelimiter)
    ) {
      if (isRowDelimiter) {
      ...
      if (rtrim) {
        if (!this._.closingQuote) {
          this._.field =
            (ref1 = this._.field) != null ? ref1.trimRight() : void 0;
        }
      }
      this._.line.push(auto_parse(this._.field || ""));
    ...
    if (
      !this._.commenting &&
      ((ref2 = this._.field) != null ? ref2.length : void 0) >
    ...
    if (
      !this._.commenting &&
      ((ref3 = this._.line) != null ? ref3.length : void 0) >
        this.options.max_limit_on_data_read
    ) {
      return Error(
        `Row delimiter not found in the file ${JSON.stringify(
          this.options.rowDelimiter
        )}`
      );
    }
  ...
  if (end) {
    if (this._.field != null) {
      if (rtrim) {
        if (!this._.closingQuote) {
          this._.field =
            (ref4 = this._.field) != null ? ref4.trimRight() : void 0;
        }
      }
      this._.line.push(auto_parse(this._.field || ""));
      this._.field = null;
    ...
    if (
      ((ref5 = this._.field) != null ? ref5.length : void 0) >
      this.options.max_limit_on_data_read
    ) {
      return Error(
        `Delimiter not found in the file ${JSON.stringify(
          this.options.delimiter
        )}`
      );
    }
...
isObjLiteral = function(_obj) {
  ...
  if (typeof _obj !== "object" || _obj === null || Array.isArray(_obj)) {
    ...
    return (function() {
      while (!false) {
        if (
          Object.getPrototypeOf((_test = Object.getPrototypeOf(_test))) === null
        ) {
          break;
        }
      }
